...
def set_attendance(request):
    if request.method == "POST":
        ...
        student_att: StudentAttendance = StudentAttendance.objects.get(
            student_id=student_id, attendance=att
        )
        ...
        student_att.save()
        if new_value in (
            ATTENDANCE_TYPE_TRUANT.value,
            ATTENDANCE_TYPE_ABSENT.value,
            ATTENDANCE_TYPE_DUTY.value,
        ):
