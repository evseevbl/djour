...
class Attendance(models.Model):
    ...
    squad = models.ForeignKey(
        "journal.Squad", models.CASCADE, blank=False, null=True, verbose_name="Взвод"
    )
    students = models.ManyToManyField(
        "journal.StudentAttendance", verbose_name="Студенты"
    )
...
class Duty(models.Model):
    ...
    attendance = models.ForeignKey(
        Attendance, models.CASCADE, blank=True, null=True, verbose_name="Дата"
    )
...
class EventParticipant(models.Model):
    student = models.ForeignKey(
        "journal.Student", models.CASCADE, verbose_name="Студент"
    )
...
class Exam(models.Model):
    SEMESTER_CHOICES = ((3, 3), (4, 4), (5, 5), (6, 6), (7, 7), (8, 8))
    NAME_EXAM = "exam"
    NAME_TEST = "test"
    NAME_CHOICES = (
        (NAME_EXAM, "Экзамен"),
    ...
    subject = models.ForeignKey(
        "journal.Subject", models.CASCADE, db_column="subject", verbose_name="Предмет"
    )
    semester = models.IntegerField("Семестр", choices=SEMESTER_CHOICES, default=0)
    squad = models.ForeignKey(
        "journal.Squad", models.CASCADE, verbose_name="Взвод", null=True
    )
    name = models.CharField(
        "Форма контроля", max_length=100, choices=NAME_CHOICES, default=""
    )
...
class Mark(models.Model):
    student = models.ForeignKey(
        "journal.Student", models.CASCADE, blank=True, null=True
    )
    ...
    lesson = models.ForeignKey(
        "journal.Lesson", models.CASCADE, blank=True, null=True, verbose_name="Предмет"
    )
...
class Penalty(models.Model):
    ...
    comment = models.CharField(
        max_length=256, blank=True, null=True, verbose_name="Комментарий"
    )
    student = models.ForeignKey(
        "journal.Student", models.CASCADE, verbose_name="Студент"
    )
...
class Squad(models.Model):
    code = models.CharField(
        unique=True, max_length=4, blank=True, null=True, verbose_name="Номер"
    )
...
class Student(models.Model):
    ...
    squad = models.ForeignKey(
        Squad, models.CASCADE, blank=True, null=True, verbose_name="Взвод"
    )
    ...
    pic = models.ImageField(
        upload_to="students_pic/", blank=True, null=True, verbose_name="Изображение"
    )
    ...
    def short(self) -> str:
        ...
        return (
            f"{self.last_name} {__get0(self.first_name)}. {__get0(self.middle_name)}."
        )
...
class Subject(models.Model):
    short = models.CharField(
        "Сокращённое название", max_length=10, blank=True, null=True
    )
...
class Lesson(models.Model):
    ...
    subject = models.ForeignKey(
        Subject, models.CASCADE, blank=True, null=True, verbose_name="Предмет"
    )
    ...
    exam = models.ForeignKey(
        "journal.Exam", models.CASCADE, blank=True, null=True, verbose_name="Экзамен"
    )
...
class PersonalInfo(models.Model):
    student = models.OneToOneField(
        Student, models.CASCADE, blank=False, null=False, verbose_name="Студент"
    )

    passport_code = models.CharField(
        "Серия паспорта", max_length=4, blank=True, null=False
    )
    passport_number = models.CharField(
        "Номер паспорта", max_length=6, blank=True, null=False
    )
    passport_issued_date = models.DateField(
        "Дата выдачи паспорта", blank=True, null=True
    )
    passport_issued = models.CharField(
        "Кем выдан паспорт", max_length=512, blank=True, null=False
    )

    reg_address = models.CharField(
        "Адрес регистрации", max_length=512, blank=True, null=False
    )
    fact_address = models.CharField(
        "Адрес фактический", max_length=512, blank=True, null=False
    )
    ...
    family_status = models.CharField(
        "Семейное положение", max_length=512, blank=True, null=False
    )
    family_members = models.CharField(
        "Члены семьи", max_length=512, blank=True, null=False
    )
    ...
    order_admission = models.CharField(
        "№ приказа о зачислении", max_length=100, blank=True, null=True
    )
    order_deduction = models.CharField(
        "№ приказа об отчислении", max_length=100, blank=True, null=True
    )
    ...
    characteristic_first_year = models.TextField(
        "Характеристика за 2 курс", blank=True, null=True
    )
    characteristic_second_year = models.TextField(
        "Характеристика за 3 курс", blank=True, null=True
    )
    characteristic_third_year = models.TextField(
        "Характеристика за 4 курс", blank=True, null=True
    )

    service_rank = models.CharField(
        "Служба в ВС, звание", max_length=100, blank=True, null=False
    )
    ...
    program = models.CharField(
        "Образовательная программа", max_length=100, blank=True, null=True
    )
...
class UserExtension(models.Model):
    ...
    squads = models.ManyToManyField(
        "journal.Squad", blank=True, verbose_name="Может редактировать взвода"
    )

    date_limit = models.BooleanField(
        default=False, verbose_name="Ограничение на текущую дату"
    )
    can_edit_attendance = models.BooleanField(
